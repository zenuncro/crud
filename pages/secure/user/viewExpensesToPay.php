...
$title = ' - Lista de Despesas a Pagar';
$user = user();
?>
            ...
            <div class="card" style="border-radius: 10px; box-shadow: 5px 5px 10px #888888; background-color:#cfdbd5;">
                <div class="card-body">
                    ...
                    <div>
                        <?php
                        $user_id = $_SESSION['id'];
                        $expenses = getTodayExpenses($user_id);

                        if (empty($expenses)) {
                            echo '<p>Nenhuma despesa encontrada.</p>';
                        } else {
                        ?>
                                    ...
                                    <tbody>
                                        ...
                                        foreach ($expenses as $expense) {
                                            ...
                                            echo '<td>
                                                ...
                                                <button class="btn btn-sm btn-success" onclick="payExpense(' . $expense['id'] . ')">Pagar</button>
                                            </td>';
                                            echo '</tr>';
                                        }
                                        ?>
    ...
    <script>
        function removeExpense(expenseId) {
            if (confirm('Tem certeza de que deseja remover esta despesa?')) {
                window.location.href = '/crud/controllers/auth/removeScheduleExpense.php?id=' + expenseId;
            }
        }

        function payExpense(expenseId) {
            if (confirm('Tem certeza de que deseja pagar esta despesa?')) {
                window.location.href = '/crud/controllers/auth/payExpense.php?id=' + expenseId;
            }
        }
    </script>
